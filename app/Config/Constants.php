...
if ((count($checkPublicFolder) == 1)  && $explodeFolder == '') {
    defined('PUBLIC_PATH') || define('PUBLIC_PATH', '');
} else if ((count($checkPublicFolder) == 2)) {
...
if (getenv('app_baseURL') || getenv('app.baseURL')) {
    ...
    if (isset($_SERVER['HTTP_HOST'])) {
        ...
        $folderProject = ($_SERVER['SERVER_PORT'] != 80) ?  $explodeFolder : ((count($checkPublicFolder) == 1) ? $explodeFolder . ''  : $checkPublicFolder[0]);
...
// 3600 second = 1 Hours 
defined('CACHE_TTL') || define('CACHE_TTL', (intval(getenv('CACHE_TTL')) ?? 3600));
