...
<section class="content">
    <div class="container-fluid">
        ...
        <form action="<?= base_url($link . '/' . $assignment->id); ?>" method="post" enctype="multipart/form-data">
            <?= csrf_field(); ?>
                                ...
                                <select type="text" class="form-control <?= ($error = validation_show_error('class_id')) ? 'border-danger' : ((old('class_id')) ? 'border-success' : ''); ?> " value="<?= old('class_id'); ?>" id="class_id" name="class_id">
                                    <?php foreach ($classes as $dt_class): ?>
                                        <?php if (old('class_id')): ?>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php if ($assignment->class_id == $dt_class->id): ?>
                                                <option selected value="<?= $dt_class->id; ?>"><?= $dt_class->name; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $dt_class->id; ?>"><?= $dt_class->name; ?></option>
                                            <?php endif; ?>
                                ...
                                <select type="text" class="form-control <?= ($error = validation_show_error('subject_id')) ? 'border-danger' : ((old('subject_id')) ? 'border-success' : ''); ?> " value="<?= old('subject_id'); ?>" id="subject_id" name="subject_id">
                                    <?php foreach ($subjects as $subject): ?>
                                        <?php if (old('subject_id')): ?>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php if ($assignment->subject_id == $subject->id): ?>
                                                <option selected value="<?= $subject->id; ?>"><?= $subject->name; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $subject->id; ?>"><?= $subject->name; ?></option>
                                            <?php endif; ?>
                                ...
                                <select type="text" class="form-control <?= ($error = validation_show_error('teacher_id')) ? 'border-danger' : ((old('teacher_id')) ? 'border-success' : ''); ?> " value="<?= old('teacher_id'); ?>" id="teacher_id" name="teacher_id">
                                    <?php foreach ($teachers as $teacher): ?>
                                        <?php if (old('teacher_id')): ?>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <?php if ($assignment->teacher_id == $teacher->id): ?>
                                                <option selected value="<?= $teacher->id; ?>"><?= $teacher->full_name; ?></option>
                                            <?php else: ?>
                                                <option value="<?= $teacher->id; ?>"><?= $teacher->full_name; ?></option>
                                            <?php endif; ?>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control <?= ($error = validation_show_error('title')) ? 'border-danger' : ((old('title')) ? 'border-success' : ''); ?>" id="title" name="title" placeholder="Title" value="<?= old('title', $assignment->title); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control <?= ($error = validation_show_error('description')) ? 'border-danger' : ((old('description')) ? 'border-success' : ''); ?>" id="description" name="description" placeholder="Description"><?= old('description', $assignment->description); ?></textarea>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control <?= ($error = validation_show_error('file')) ? 'border-danger' : ((old('file')) ? 'border-success' : ''); ?>" id="file" name="file" placeholder="file" value="<?= old('file', $assignment->file); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" class="form-control <?= ($error = validation_show_error('deadline')) ? 'border-danger' : ((old('deadline')) ? 'border-success' : ''); ?>" id="deadline" name="deadline" placeholder="deadline" value="<?= old('deadline', $assignment->deadline); ?>">
                            </div>
                            <?= ($error) ? '<div class="error text-danger mb-2" style="margin-top: -15px">' . $error . '</div>' : ''; ?>
